...
function RunDevEnv()
  ...
  if build_dir then
    ...
    if vim.fn.filereadable(build_dir .. '\\main.exe') == 1 then
      print('Found main.exe in: ' .. build_dir)
      vim.cmd '!devenv main.exe'
    else
      print 'main.exe not found in the build directory.'
    end
  else
...
vim.keymap.set('n', '<F5>', ':lua RunDevEnv()<CR>')
vim.keymap.set('n', '<C-B>', ':lua RunBuildBat()<CR>')
